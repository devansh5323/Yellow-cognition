...
import { LayoutDashboard, Menu, X, LogOut, GraduationCap } from "lucide-react";
...
const NAV = [{ to: "/district/dashboard", label: "Overview", icon: LayoutDashboard }] as const;

const EASE = [0.2, 0.7, 0.2, 1] as const;

export function DistrictAppShell({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (to: string) =>
    pathname === to || (to !== "/district/dashboard" && pathname.startsWith(to));
            ...
            {NAV.map((item) => {
              ...
              return (
                ...
                >
                  {active && (
                    <motion.span
                      layoutId={reduce ? undefined : "district-nav-rail"}
                  ...
                  {active && (
                    <motion.span
                      layoutId={reduce ? undefined : "district-nav-dot"}
            ...
            >
              <div className="relative shrink-0">
                <Avatar className="h-9 w-9 border border-border">
                  <AvatarFallback className="bg-gradient-to-br from-[hsl(28_88%_60%)] to-[hsl(0_78%_60%)] text-white text-[11px] font-heading font-extrabold">
                    {session?.initials ?? "DL"}
                  </AvatarFallback>
                </Avatar>
                <span
