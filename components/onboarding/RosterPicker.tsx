...
const ROSTER_OPTIONS: {
  ...
  { id: "invite", label: "Send parent app invite", sub: "Parents download the app & create the child's profile", Icon: Send, eta: "~1m / family" },
  { id: "csv", label: "Upload a CSV", sub: "Drop a roster export from any SIS", Icon: Upload, eta: "~30s" },
  { id: "manual", label: "Add students manually", sub: "Enter each child + parent contact, send invites", Icon: UserPlus, eta: "Per child" },
  { id: "sample", label: "Start with a sample class", sub: "Explore with 24 demo students first", Icon: Sparkles, eta: "Instant" },
];
...
const INTEGRATIONS: {
  ...
  label: string;
  sub: string;
  rosterMethod: RosterMethod | null;
  Logo: () => React.ReactElement;
}[] = [
  {
    id: "faria",
    label: "Faria",
    sub: "Sync from your Faria school directory",
    rosterMethod: null,
    Logo: FariaLogo,
  },
  {
    id: "powerschool",
    label: "PowerSchool",
    sub: "Pull rosters from your PowerSchool SIS",
    rosterMethod: null,
  ...
  },
  {
    ...
    sub: "Sync students with one click",
    rosterMethod: "google",
  ...
  },
  {
    id: "pupilpod",
    label: "Pupilpod",
    sub: "Import your existing Pupilpod class",
    rosterMethod: null,
    Logo: PupilpodLogo,
  },
];

const ROSTER_LABELS: Record<RosterMethod, string> = {
  invite: "Parent app invite",
  csv: "From CSV",
...
export function rosterMethodLabel(m: RosterMethod | null | undefined): string {
  if (!m) return "Not set";
  return ROSTER_LABELS[m];
}
...
export function RosterPicker({
  ...
  value: RosterMethod | null;
  onChange: (m: RosterMethod) => void;
}) {
  return (
      ...
      <div className="space-y-3">
        ...
        {value === "csv" && (
          <motion.div
            ...
            className="mt-2 rounded-2xl border-2 border-dashed border-primary/30 bg-primary/[0.04] p-6 text-center"
          >
            <Upload className="h-5 w-5 mx-auto text-primary" />
            <div className="mt-2 font-heading font-bold text-[13.5px]">Drop your roster here</div>
            <div className="text-[11.5px] text-muted-foreground mt-0.5">
              CSV with names + emails works best · we'll preview before saving
            </div>
          </motion.div>
        )}
      </div>
...
function IntegrationsSection({
  ...
  const selectedIntegration = INTEGRATIONS.find(
    (it) => it.rosterMethod !== null && value === it.rosterMethod,
  );
    ...
    <div className="rounded-2xl border border-border/70 bg-card/60 overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {INTEGRATIONS.map((it) => {
            const Logo = it.Logo;
            const enabled = it.rosterMethod !== null;
            const active = enabled && value === it.rosterMethod;
            return (
              <span
                key={it.id}
                title={it.label}
          ...
          >
            <div className="border-t border-border/60 p-3 grid grid-cols-2 gap-2.5">
              {INTEGRATIONS.map((it) => {
                ...
                const active = enabled && value === it.rosterMethod;
                const handleClick = () => {
                  if (enabled && it.rosterMethod) onChange(it.rosterMethod);
                };
                return (
                  <button
                    key={it.id}
...
/* ─────────────────────────────────────────────────────────
 * Brand logos — inline SVG so we don't ship trademarked PNGs.
 * ───────────────────────────────────────────────────────── */
function FariaLogo() {
  return (
    <svg viewBox="0 0 40 40" className="h-full w-full block" aria-hidden>
      <defs>
        ...
        <linearGradient id="fa-fg" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stopColor="#FFD27A" />
          <stop offset="1" stopColor="#F06AA8" />
        </linearGradient>
      </defs>
      <rect x="0" y="0" width="40" height="40" rx="8" fill="url(#fa-bg)" />
      <path
        d="M16 10h12v5h-7v5h6v5h-6v8h-5z"
        fill="url(#fa-fg)"
      />
    </svg>
  );
