...
import { DISRUPTION_HUE, type BehaviorSupport, type SupportStatus } from "@/lib/classBehavior";
...
export function BehaviorSupportTable({ items }: { items: BehaviorSupport[] }) {
  ...
  const rows = useMemo(() => {
    if (filter === "support-only") {
      // Top of the list — students with a low behaviour pillar or already on a plan.
            ...
            {rows.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3 align-middle">
                    <PatternPill primary={item.primary} label={item.primaryLabel} />
                  </td>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={7} className="p-10 text-center text-[12.5px] text-muted-foreground">
                  No students need behaviour support right now.
                </td>
              </tr>
            )}
          </tbody>
...
function PatternPill({ primary, label }: { primary: BehaviorSupport["primary"]; label: string }) {
  const tone = DISRUPTION_HUE[primary];
  return (
    <span
...
function StatusPill({ resolved, status }: { resolved: boolean; status: SupportStatus }) {
  if (resolved) {
  ...
  const STATUS_TONE: Record<SupportStatus, { tone: string; label: string }> = {
