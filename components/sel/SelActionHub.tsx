...
import type { SelActionItem, SelActionPriority } from "@/lib/selPulse";

const EASE = [0.2, 0.7, 0.2, 1] as const;

const PRIORITY_LABEL: Record<SelActionPriority, string> = {
  high: "High",
  medium: "Medium",
  low: "Low",
};

const PRIORITY_TONE: Record<SelActionPriority, string> = {
  high: "hsl(0 78% 58%)",
  medium: "hsl(38 92% 50%)",
  low: "hsl(142 55% 45%)",
};

export function SelActionHub({ items }: { items: SelActionItem[] }) {
    ...
    >
      ...
      {items.length === 0 ? (
        <p className="text-[12px] text-muted-foreground">
          Nothing flagged right now — every active pulse is holding steady.
        </p>
      ) : (
