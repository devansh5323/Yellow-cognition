...
import { FOCUS_CHECKIN_QUESTIONS, focusCheckInFrictionScore } from "@/lib/classFocus";
...
export function MonthlyFocusCheckIn() {
  ...
  const total = FOCUS_CHECKIN_QUESTIONS.length;
  ...
  const current = FOCUS_CHECKIN_QUESTIONS[step];
  const allAnswered = answeredCount === total;

  const result = useMemo(() => focusCheckInFrictionScore(answers), [answers]);
  ...
  return (
    <section
      aria-label="Monthly focus check-in"
      ...
      <div
        ...
        style={{
          background: submitted
            ...
            : "radial-gradient(60% 50% at 0% 0%, hsl(38 92% 80% / 0.30), transparent 65%), radial-gradient(60% 50% at 100% 20%, hsl(258 70% 80% / 0.20), transparent 70%)",
        }}
      />
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-1 font-heading font-extrabold text-[20px] md:text-[22px] leading-tight">
                {submitted
                  ...
                  : "Capture this month's classroom friction"}
              </h2>
              <p className="mt-1 text-[12.5px] text-muted-foreground max-w-xl">
                {submitted
                  ? "Your monthly read powers the focus insights above. Come back next month to refresh."
                  : "8 quick MCQs — under 90 seconds. Captures what gameplay alone can't see."}
              </p>
            </div>
          </div>
          ...
          {submitted ? (
            ...
            >
              <div className="rounded-2xl border border-border/70 bg-background/40 p-4 flex flex-col justify-center">
                ...
                <p className="mt-2 text-[12px] text-muted-foreground leading-snug">
                  Higher means smoother classroom flow. We blend this with PFI to compute your class
                  focus score.
                </p>
              </div>
            ...
            >
              ...
              <ol className="mt-5 flex items-center gap-1.5 flex-wrap">
                {FOCUS_CHECKIN_QUESTIONS.map((q, i) => {
