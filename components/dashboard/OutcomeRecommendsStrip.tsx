...
import { ArrowUpRight, ClipboardCheck, Sparkle, Sparkles, Users } from "lucide-react";
import type { OutcomeRecommendation, RecommendationKind } from "@/lib/learningOutcomes";
import { cn } from "@/lib/utils";

const EASE = [0.2, 0.7, 0.2, 1] as const;

const KIND_TONE: Record<RecommendationKind, string> = {
  "Whole Class": "hsl(258 55% 60%)",
  "Small Group": "hsl(196 75% 50%)",
  "Quick Check": "hsl(38 92% 55%)",
};

const KIND_ICON: Record<RecommendationKind, typeof Users> = {
  "Whole Class": Users,
  "Small Group": Sparkle,
  "Quick Check": ClipboardCheck,
};

export function OutcomeRecommendsStrip({
  recommendations,
}: {
  ...
  // Synthetic "relevance" score, scaled so that more concrete recommendations
  // read as a higher-confidence AI synthesis. Prototype-only — replace with a
  // real model output when wired.
  const relevance = useMemo(
    () => Math.min(96, 84 + recommendations.length * 3),
  ...
  return (
    ...
    >
      {/* Atmospheric backdrop — soft amber + lavender tint to read as AI */}
      <div
        aria-hidden
        ...
        <header className="flex items-start justify-between gap-2.5">
          <div className="flex items-start gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11.5px] text-muted-foreground mt-0.5">Tailored to this class</p>
            </div>
          </div>
