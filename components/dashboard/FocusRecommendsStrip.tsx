...
import {
  ...
  type FocusRecommendKind,
} from "@/lib/classFocus";
import { cn } from "@/lib/utils";

const EASE = [0.2, 0.7, 0.2, 1] as const;

const KIND_TONE: Record<FocusRecommendKind, string> = {
  "Whole Class": "hsl(258 55% 60%)",
  "Small Group": "hsl(196 75% 50%)",
  "Hero Activity": "hsl(38 92% 52%)",
};

const KIND_ICON: Record<FocusRecommendKind, LucideIcon> = {
  "Whole Class": Users,
  "Small Group": ClipboardCheck,
  "Hero Activity": Trophy,
};
...
export function FocusRecommendsStrip({
  ...
  const relevance = useMemo(
    () => Math.min(96, 82 + recommendations.length * 3),
    [recommendations.length],
  );

  return (
    <section
      aria-label="Yellow Recommends — Focus"
          ...
          <div className="flex items-start gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11.5px] text-muted-foreground mt-0.5">
                Focus actions and Attention Hero activities
              </p>
            </div>
          </div>
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {r.targets.map((t) => (
                        <span
                          ...
                          style={{
                            color: FOCUS_DOMAIN_HUE[t],
                            background: `color-mix(in srgb, ${FOCUS_DOMAIN_HUE[t]} 10%, transparent)`,
                            border: `1px solid color-mix(in srgb, ${FOCUS_DOMAIN_HUE[t]} 22%, transparent)`,
                          }}
                        >
                          {FOCUS_DOMAIN_LABEL[t]}
                        </span>
                      ))}
                    </div>
                  )}
