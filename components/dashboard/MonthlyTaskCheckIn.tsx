...
import { TASK_CHECKIN_QUESTIONS, taskCheckInScore } from "@/lib/classTask";
...
export function MonthlyTaskCheckIn() {
  const reduce = useReducedMotion();
  const total = TASK_CHECKIN_QUESTIONS.length;
  ...
  const current = TASK_CHECKIN_QUESTIONS[step];
  const allAnswered = answeredCount === total;

  const result = useMemo(() => taskCheckInScore(answers), [answers]);
  ...
  return (
    <section
      aria-label="Monthly task engagement check-in"
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-1 font-heading font-extrabold text-[20px] md:text-[22px] leading-tight">
                {submitted
                  ...
                  : "Capture this month's task engagement"}
              </h2>
          ...
          {submitted ? (
            ...
            >
              <div className="rounded-2xl border border-border/70 bg-background/40 p-4 flex flex-col justify-center">
                ...
                <p className="mt-2 text-[12px] text-muted-foreground leading-snug">
                  Higher means smoother engagement. We blend this with completion signals to set the
            ...
            >
              ...
              <ol className="mt-5 flex items-center gap-1.5 flex-wrap">
                {TASK_CHECKIN_QUESTIONS.map((q, i) => {
